...
TH1F *hinvW[14];TH1F *hinvE[14];
static const double bin_w[15]={0.5,0.6,0.7,0.8,0.9,1.0,1.1,1.2,1.3,1.4,1.6,1.8,2.0,2.2,2.4};
static const double bin_L[9]={0.6,0.9,1.2,1.6,2.0,2.4,2.8,3.2,3.8};
int pnL=8;
...
for(int n=0;n<pnL;n++){
sprintf(strE,"%s%d","inv2E",n);
sprintf(strW,"%s%d","inv2W",n);
const char *WestH=(char*)strW;
const char *EastH=(char*)strE;
hinvW[n]=new TH1F(WestH,"inv mass^{2} for Tof.West",150,0.98,1.28);
hinvE[n]=new TH1F(EastH,"inv mass^{2} for Tof.East",150,0.98,1.28); }
...
for (Long64_t jentry=0; jentry<nentries;jentry++) 
   {NN+=1;
        ...
        if(ientry%100000==0) {cout << ientry<< " all entri is "<< NN <<" the naber of TRee is "<< NNTREE<<endl;}
        nb = fChain->GetEntry(jentry);  nbytes += nb;


if(fabs(bbcz)<30 && cent>0 && cent<60 )
{
    for (int i=0;i<mh;i++)
    {  
...
for(int n=0;n<pn;n++){
        ...
        if ( pt>bin_w[n] && pt<bin_w[n+1] && sigtof[i]<3 && pltof[i]>0 && pt>0.8 && pt<1.8) 
        {   
...
void MTREE::INV()
{
float m12,mk,mi,pti,ptk,pxi,pxk,pyi,pyk,pzi,pzk,Pt;
...
for (Long64_t jentry=0; jentry<nentries;jentry++) 
   {NN+=1;
    ...
    if(fabs(bbcz)<30 && cent>20 && cent<50 )
        {
        for (int i=0;i<mh;i++)
            {  
            pti=p[i]*sin(the0[i]);mi=pow(p[i],2)*(pow(ttof[i]*(1e-7)*CC/pltof[i],2)-1);
...
Pt=sqrt(pow((pxi+pxk),2)+pow((pyi+pyk),2));Px=pxi+pxk;Py=pyi+pyk;Pz=pzi+pzk;
	...
	bool A=(charge[i]==-1 && dcarm[i]==0 && fabs(IsPionE(mi,pti))<3 && fabs(IsProtonE(mk,ptk))<3&& fabs(IsKaonE(mi,pti))>3);
	bool C=(charge[i]!=charge[k] && dcarm[i]==dcarm[k] && ptk>PtMin &&pti>PtMin && pti<PtMax && ptk<PtMax && etof[k]>0.002 && etof[i]>0.002);
            if (A && C){ 
Ei=sqrt(p[i]*p[i]+Mpi);Ek=sqrt(p[k]*p[k]+Mpr);m12=sqrt(pow((Ei+Ek),2)-(Px*Px+Py*Py+Pz*Pz));
for(int n=0;n<pnL;n++){
 
        if ( Pt>bin_L[n] && Pt<bin_L[n+1]) 
            {hinvE[n]->Fill(m12);};
        };

if (Pt>2.4){invE->Fill(m12);invALL->Fill(m12);}}}; 
        }   }
...
Pt=sqrt(pow((pxi+pxk),2)+pow((pyi+pyk),2));Px=pxi+pxk;Py=pyi+pyk;Pz=pzi+pzk;
        ...
        bool A=(charge[i]==-1 && dcarm[i]==1 && fabs(IsPionW(mi,pti))<3 && fabs(IsProtonW(mk,ptk))<3 && fabs(IsKaonW(mi,pti))>3);
        bool C=(charge[i]!=charge[k] && dcarm[i]==dcarm[k] && ptk>PtMin &&pti>PtMin && pti<PtMax && ptk<PtMax && etof[k]>60 && etof[i]>60 && etof[i]<600 && etof[k]<600);
            if (A && C){
Ei=sqrt(p[i]*p[i]+Mpi);Ek=sqrt(p[k]*p[k]+Mpr);m12=sqrt(pow((Ei+Ek),2)-(Px*Px+Py*Py+Pz*Pz));
for(int n=0;n<pnL;n++){
...
if (Pt>2.4){invW->Fill(m12);invALL->Fill(m12);}}}; 
        }   }
...
line1->DrawLine(1.11568,0,1.11568,invE->GetMaximum());
MyW1 -> cd(2);invW->GetXaxis()->SetTitle("inv mass,Gev");invW->Draw();
TLine *line2=new TLine();
d_outfileinv = new TFile(outfile,"recreate");
line2->DrawLine(1.11568,0,1.11568,invW->GetMaximum());
d_outfileinv->cd();
